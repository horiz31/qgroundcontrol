...
PreFlightCheckButton {
    ...
    specifiedBottomPadding: telemetryFailure ? (Math.round(ScreenTools.defaultFontPixelHeight / 2) + calibrateButton.height + Math.round(ScreenTools.defaultFontPixelHeight / 2)) : Math.round(ScreenTools.defaultFontPixelHeight / 2)
    ...
    property string   _buttonLabel:   qsTr("Calibrate WindSpeed")
    ...
    function updateTelemetryTextFailure() {
        if((_airSpeed > _airSpeedLimit) || _windOffsetFailure) {
            if (_airSpeed > _airSpeedLimit)               telemetryTextFailure = qsTr("Failure. Wind speed measures " + _airSpeed.toFixed(1) + " m/s, which is above the maximum of 3 m/s. Please cover the sensor and restart the vehicle to recalibrate.")
